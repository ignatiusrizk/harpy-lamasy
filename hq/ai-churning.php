...
async function loadList(){
  ...
  try {
    const r = await fetch(`/hq/ai-churning.php?action=list&outlet_id=${outletId}&limit=${limit}`);
...
async function generateMsg(c) {
  ...
  try {
    const r = await fetch('/hq/ai-churning.php?action=generate', { method:'POST', body: fd });
    const d = await r.json();
    if (d.error) {
      btn.disabled = false;
...
async function markStatus(logId, status, card) {
  ...
  try {
    await fetch('/hq/ai-churning.php?action=mark', { method:'POST', body: fd });
  } catch (e) {}
