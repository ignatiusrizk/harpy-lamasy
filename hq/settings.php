...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_notif'])) {
    if (!hash_equals(getCsrfToken(), $_POST['_csrf'] ?? '')) {
        ...
        $alerts = ['coin_low', 'trial_ending', 'daily_report', 'alert_anomali'];
        $channels = ['email', 'wa'];
        $prefs = [];
        foreach ($alerts as $a) {
...
async function loadTopupHistory(){
  ...
  try {
    const r = await fetch('/hq/settings.php?action=topup_history');
...
async function loadCoinUsage(){
  ...
  try {
    const r = await fetch('/hq/settings.php?action=coin_usage');
    const rows = await r.json();
