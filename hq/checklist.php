...
async function loadTemplates(){
  ...
  try {
    const r = await fetch('/hq/checklist.php?action=templates');
...
async function saveTpl(){
  ...
  try {
    const r = await fetch('/hq/checklist.php?action=save', {method:'POST', body:JSON.stringify(body)});
    const d = await r.json();
...
async function loadCompliance(){
  ...
  try {
    const r = await fetch(`/hq/checklist.php?action=compliance&tanggal=${tgl}`);
    const d = await r.json();
    if (d.error){ wrap.innerHTML = `<div class="empty">⚠️ ${esc(d.error)}</div>`; return; }
