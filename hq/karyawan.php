...
async function loadFilterOptions(){
  const r = await fetch('/hq/karyawan.php?action=filter_options');
...
async function loadList(){
  ...
  const url = '/hq/karyawan.php?action=list&q=' + encodeURIComponent(q)
...
async function loadPerformance(kid){
  ...
  if (!box) return;
  try {
    const r = await fetch('/hq/karyawan.php?action=performance&id=' + kid);
    const d = await r.json();
