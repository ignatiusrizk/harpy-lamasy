...
const content = [
  ...
  dataTable(
    ...
    [
      ...
      ["Auth", "Session + CSRF (random per-sesi)", "HMAC-SHA256 utk token switch-outlet"],
      ["AI", "Anthropic Claude", "claude-sonnet-4-x, claude-haiku-4-5"],
      ["Email", "Hostinger SMTP via PHPMailer", "noreply@harpy.id"],
      ["WhatsApp", "wa.me click-to-chat link", "Semi-manual (staf klik kirim), bukan API blast"],
  ...
  ...numbered_list([
    ...
    "DB INSERT tenants + outlets + hl_users; auto-credit 10.000 coin trial",
  ...
  ...numbered_list([
    ...
    "Staff produksi scan QR di /produksi.php → input tiap tahap + signature + foto",
    "Tahap (status_proses): Masuk → Cuci → Kering → Setrika → Siap",
    "Status proses auto-update: masuk → cuci → kering → setrika → siap",
  ...
  ...bullet("saas_packages — subscription tiers"),
  ...
  h3("Master data"),
  ...bullet("outlets, hl_layanan, hl_layanan_master, hl_parfum"),
  ...
  ...bullet("CSRF token per sesi (random_bytes(32)), dicek verifyCsrf()"),
  ...
  ...bullet("superadmin_logs — SA-level action log"),
  ...
  dataTable(
    ...
    [
      ...
      ["WhatsApp", "wa.me link (click-to-chat)", "Order siap, tagih piutang, dll — staf klik kirim", "—"],
  ...
  ...bullet("Anthropic API key di env config (WA via wa.me link, tanpa API key)"),
